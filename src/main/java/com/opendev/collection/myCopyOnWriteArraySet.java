...
public class myCopyOnWriteArraySet {
    ...
    public static void main(String[] args) {
         ...
         * 和CopyOnWriteArrayList类似，其实CopyOnWriteSet底层包含一个CopyOnWriteList，
         ...
         *   2. CopyOnWriteArraySet包含CopyOnWriteArrayList对象
         *   它是通过CopyOnWriteArrayList实现的。而CopyOnWriteArrayList本质是个动态数组队列，
         * 所以CopyOnWriteArraySet相当于通过通过动态数组实现的“集合”！
         * CopyOnWriteArrayList中允许有重复的元素；但是，CopyOnWriteArraySet是一个集合，
         * 所以它不能有重复集合。因此，CopyOnWriteArrayList额外提供了addIfAbsent()和addAllAbsent()这两个添加元素的API，
         * 通过这些API来添加元素时，只有当元素不存在时才执行添加操作！
         * 至于CopyOnWriteArraySet的“线程安全”机制，和CopyOnWriteArrayList一样，
         * 是通过volatile和互斥锁来实现的。这个在前一章节介绍CopyOnWriteArrayList时数据结构时，
         * 已经进行了说明，这里就不再重复叙述了。
         *
         *
         *
