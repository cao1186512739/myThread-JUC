...
public class myCopyOnWriteArrayList {
    public static void main(String[] args) {
         ...
         * 由于读操作根本不会修改原有的数据，因此如果每次读取都进行加锁操作，
         * 其实是一种资源浪费。我们应该允许多个线程同时访问 List 的内部数据，毕竟读操作是线程安全的。
         *
         * 这和 ReentrantReadWriteLock 读写锁的思想非常类似，也就是
         * 读读共享、写写互斥、读写互斥、写读互斥。JDK中提供了 CopyOnWriteArrayList 类，
         ...
         * CopyOnWriteArrayList 类的所有可变操作（add，set等等）
         * 都是通过创建底层数组的新副本来实现的。当 List 需要被修改的时候，
         * 并不直接修改原有数组对象，而是对原有数据进行一次拷贝，将修改的内容写入副本中。
         * 写完之后，再将修改完的副本替换成原来的数据，这样就可以保证写操作不会影响读操作了。
         *
         * 从 CopyOnWriteArrayList 的名字可以看出，
         * CopyOnWriteArrayList 是满足 CopyOnWrite 的 ArrayList，
         * 所谓 CopyOnWrite 的意思：、就是对一块内存进行修改时，
         * 不直接在原有内存块中进行写操作，而是将内存拷贝一份，在新的内存中进行写操作，
